...
Now you should be able to perform CRUD operation with Redis commands. For example, you can insert data to Redis via SET command and the fetch it via GET command. The above redis client command might require password if you have setup authentication in your redis configuration file. If Redis password is not set, then you can connect to Redis instance flawlessly. Be default, Redis uses the port 6379 that can be modified in the redis configuration file.
