...
Redis is an open source, in-memory, key-value data store most commonly used as a primary database, cache, message broker, and queue. Redis delivers sub-millisecond response times, enabling fast and powerful real-time applications in industries such as gaming, fintech, ad-tech, social media, healthcare, and IoT. You can run a Redis database directly over your local mac os or in a container. If you have Docker installed in your sytem, type the below command:

```bash
 docker run -d -p 6379:6379 redislabs/redismod
```

You can connect to Redis server using the below redis client command:

```
 redis-cli -p 6379
```

The above command will make a connection to Redis server. It will then  return a prompt that allows you to run redis command and get some data value.
