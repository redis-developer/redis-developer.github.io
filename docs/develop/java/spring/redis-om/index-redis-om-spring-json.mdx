...
adds JSON as a native Redis datatype `ReJSON-RL` and it is seamlessly integrated
...
Redis OM Spring relies on the power of [Redis Stack](https://redis.io/docs/stack/about/).
...
apply custom mapping strategies, and use secondary indexes maintained by Redis.
...
the `JSON.SET` commands is used to save the JSON serialization of the entity. Once that operation succeeds, the
...
characters, say you try `red` or `RED` you will get no hits. Redis limits the minimun string match size to 4
