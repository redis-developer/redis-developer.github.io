...
title: 12 Redis Anti-Patterns Every Developer Should Avoid
sidebar_label: 12 Redis Anti-Patterns Every Developer Should Avoid
...
"Antipatterns" basically refers to those practices and solutions that might seem to be a good fit initially but when it comes to implementation phase, it makes your code much more complex. Let us look at the top 12 Redis anti-patterns to avoid:
...
Using the OSS Cluster API, the connection to the nodes are maintained by the client as needed, so you’ll have multiple connections open to different nodes at any given time. 

:::important
With Redis Enterprise, the connection is actually to a proxy, which takes care of the complexity of connections at the cluster level.
:::
...
### (Bonus) - Translating a table or JSON to a HASH without considering query pattern
