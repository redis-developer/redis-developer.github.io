...
Please note that this code is open source and implements basic feature of live chat app. You can find the link at the end of this tutorial..
...
To implement the frontend of the chatting app, run the below command:
...
```
You can now access chat window in the browser.
...
To access the fully working chat app,where you can type chat message run the below commands:
...
- Rooms which user belongs too are stored at user:{userId}:rooms as a set of chat room ids. A room is added by SADD user:{userId}:rooms {roomId} command.
