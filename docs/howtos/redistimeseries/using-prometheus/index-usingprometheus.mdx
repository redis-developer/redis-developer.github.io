...
Prometheus is an open-source systems monitoring and alerting toolkit. It  collects and stores its metrics as time series data, i.e. metrics information.  The metrics are numeric measurements, time series meaning that changes recorded over time. These metrics are stored with the timestamp at which it was recorded, alongside optional key-value pairs called labels. Metrics play an important role in understanding why your application is working in a certain way.

### Prometheus remote storage adapter for RedisTimeSeries

In the [RedisTimeSeries ](https://github.com/RedisTimeSeries)organization you can find projects that help you integrate RedisTimeSeries with other tools, including Prometheus and Grafana. Prometheus remote storage adapter for RedisTimeSeries is available and the project is hosted over [https://github.com/RedisTimeSeries/prometheus-redistimeseries-adapter](https://github.com/RedisTimeSeries/prometheus-redistimeseries-adapter).  It is basically a read/write adapter to use RedisTimeSeries as a backend database. Redis TimeSeries Adapter receives [Prometheus](https://prometheus.io/) metrics via the [remote write](https://prometheus.io/docs/prometheus/latest/storage/#remote-storage-integrations), and writes to [Redis with TimeSeries module](https://github.com/RedisLabsModules/redis-timeseries).
...
The `adapter` service directly uses an image “`redislabs/prometheus-redistimeseries-adapter:master`” that’s pulled from Docker Hub. Sets the default command for the container:  `-redis-address", "redis:6379 and listen to the address 0.0.0.0:9201. `
...
Open [http://hostIP:3000](http://host:3000) to access the Grafana dashboard. The default username and password is “admin/admin”.
...
It’s time to test drive a few demo scripts built by the Redis Inc. team. To start with, clone the below repository
