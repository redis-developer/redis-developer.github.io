...
This pop-up store demo illustrates a company that sells a single product and has 10,000 units available for purchase. Each customer can purchase one unit and the sale lasts only 10 minutes, so order processing must be instantaneous. The demo shows how to visualize data pipeline in real-time using [Redis Streams](https://redis.io/topics/streams-intro), Redis Time Series, RedisGears and [Redis Datasource with Grafana](https://developer.redis.com/explore/redisdatasource).
