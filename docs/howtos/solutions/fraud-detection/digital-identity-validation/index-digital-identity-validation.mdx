...
async function listenToStreams(
  ...
  try {
    ...
    await Promise.all(
      streams.map((stream) => {
        return redis.xGroupCreate(stream.streamKeyName, groupName, idPosition, {
          MKSTREAM: true,
        });
      }),
    );
  ...
  while (true) {
    ...
    const dataArr = await redis.xReadGroup(
      commandOptions({
        isolated: true,
      }),
      groupName,
    ...
    if (dataArr && dataArr.length) {
      for (let data of dataArr) {
        for (let messageItem of data.messages) {
          ...
          const stream = streams.find((s) => s.streamKeyName == streamKeyName);
          ...
          redis.xAck(streamKeyName, groupName, messageItem.id);
        }
      }
    }
  }
