...
- **Lookup pattern**: Entire products can be pre-cached in Redis and the application can perform any product query on Redis similar to primary database
- **Cache-aside** pattern: Redis is filled on-demand based on whatever search parameters are requested by the frontend.

This tutorial focuses on the **cache-aside** pattern, which operates as follows:

![cache-aside with Redis and MongoDB](images/cache-aside.jpg)

The goal of this design pattern is to set up **optimal** caching (load as you go) for better read operations.

To understand how the cache-aside pattern functions, consider the following process:
