...
![Initial CQRS architecture](images/microservices-architecture-with-redis.png)
...
In the code above, we check for the authorization header, else we create a new one and store it in Redis. Then we retrieve the session from Redis. Further down the line we attach the session to an `x-session` header prior to calling into the orders service. Now let's see how the orders service receives the session.
...
The last line above shows how to pull the session out of the `x-session` header and get the `userId`.
