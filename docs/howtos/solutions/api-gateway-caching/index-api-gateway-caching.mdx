...
The example above is not meant to be the defacto best practice for handling authentication. It is meant to illustrate what you might do with respect to Redis.

:::

In the code above, we check for the authorization header, else we crreate a new one and store it in Redis. Then we retrieve the session from Redis. Further down the line we attach the session to an `x-session` header prior to calling into the orders service.
