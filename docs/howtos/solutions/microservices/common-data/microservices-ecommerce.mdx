import SourceCode from './microservices-source-code-tip.mdx';
...
- `Shopping Cart`: Add products to the cart, then checkout using the "Buy Now" button.
  ![redis microservices e-commerce app frontend shopping cart](images/design-cart-2.png)

- `Order history`: Once an order is placed, the `Orders` link in the top navigation bar will show you the order status and history
