...
You will see that we are first adding the order to Redis, then to MongoDB. While this works, you might also consider using Redis Gears to handle database synchronization in the background. For the purposes of this tutorial we are not going to outline exactly how you might do that, but instead focus on how the data is stored and accessed in Redis.
