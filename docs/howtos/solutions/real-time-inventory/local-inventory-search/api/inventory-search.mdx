The code that follows shows an example API request and response for inventorySearch activity.
...
When you make a request, it goes through the API gateway to the inventory service. Ultimately, it ends up calling a `inventorySearch` function which looks as follows:
