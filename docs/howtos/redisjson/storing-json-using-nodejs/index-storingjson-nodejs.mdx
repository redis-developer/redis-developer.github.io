...
title: How to store and retrieve JSON document using NodeJS 
sidebar_label: Storing and retrieving JSON document using NodeJS
...
To demonstrate RedisJSON with NodeJS, let us consider a sample social network application.Imagine that you're building a sort of social network application where users can "check in" at different locations and give them a star rating… from 0 for an awful experience through 5 to report that they had the best time ever there! When designing your application, you determined that there's a need to manage data about three main entities:
