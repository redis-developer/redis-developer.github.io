...
By default, the Redis Stack Docker containers use internal configuration files for Redis. To start Redis with a local configuration file, you can use the `-v` volume options:
...
[Follow this tutorial](/howtos/redisjson/getting-started) to learn how to store and query JSON documents using Redis Stack.
...
Follow this tutorial to learn [how to implement low latency and compact Probabilistic data structures](/howtos/redisbloom) using Redis Stack.
