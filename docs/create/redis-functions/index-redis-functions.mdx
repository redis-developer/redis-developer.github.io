...
One of the basic rules of software development is that you can not rely on user input, so let’s make sure we don’t do that. If a user creates a string named `my_string` and tries to run the touch function on it they will get an error:
