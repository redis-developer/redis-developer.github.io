...
- [RedisJSON](https://developer.redis.com/howtos/redisjson/getting-started): used to store, update and fetch JSON values from Redis keys.
...
- [AWS Lambda](https://aws.amazon.com/lambda/): used a serverless compute service that runs your code in response events and manages the underlying compute service automatically for you.
...
npm install is a npm cli-command that does the predefined thing i.e install dependencies specified inside package.json
