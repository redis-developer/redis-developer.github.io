...
[Google Cloud Run](https://cloud.google.com/run/docs) allows you to deploy and scale serverless HTTP containers without worrying about provisioning servers, scaling servers up and down to meet demands, or overpaying when you are allocated with more resources than necessary. It makes container deployment much easier. It is good for developing software in cloud applications, hence delivering web apps, [API,](https://cloud.google.com/run/docs/reference/rest) background jobs, etc.
...
### Compelling feature of Google Cloud Run
...
In this tutorial, you will learn how to deploy a simple Redis Rating limiting application to Google Cloud Run in just 5 minutes.
...
Visit [developer.redis.com/create/rediscloud/](https://developer.redis.com/create/rediscloud/) and create [a free Redis Enterprise Cloud account](https://redis.com/try-free/). Once you complete the signup tutorial, you will be provided with the database endpoint URL and password. Save it for future reference.




![alt_text](images/rediscloud_register.png "image_tooltip")


### Step 2. Install Google Cloud CLI

Run the below command to install Google Cloud CLI Core Libraries and dependencies
...
To deploy your app with the gcloud CLI, you must download, install, and initialize the [gcloud CLI](https://cloud.google.com/sdk/docs). 
...
![alt_text](images/authenticate_gcloud.png "image_tooltip")
...
![alt_text](images/gcloud1.png "image_tooltip")
...
![alt_text](images/gcloud5.png "image_tooltip")
...
![alt_text](images/gcloud6.png "image_tooltip")
...
![alt_text](images/ratelimit.png "image_tooltip")


This app allows you to choose the desired requests every 10 seconds. The app blocks connections from a client after surpassing a certain amount of requests (default: 10) per time (default: 10 sec). That will let the user know how many requests they have remaining before they run over the limit. On the tenth run, the server should return an HTTP status code of 429 (“Too Many Requests”).
