...
- Heroku CLI: It is a tool for building and running Heroku app from within the terminal. (Docker, too, uses its own CLI for working with the platform.) Learn about [Heroku Dev Center](https://devcenter.heroku.com/)
...
Creating a heroku account is free of cost.
...
If you are using Heroku for the first time, visit Heroku website and create your new Heroku account [through this link](https://signup.heroku.com/login).
...
The Heroku Command Line Interface (CLI) lets you create and manage Heroku apps directly from the terminal. It's an essential part of using Heroku. In order to install Heroku CLI, run the below command:
...
Use the following Heroku Command Line interface to login to Heroku dashboard:
...
The `basic-redis-chat-demo-go` is the name of the project directory.
