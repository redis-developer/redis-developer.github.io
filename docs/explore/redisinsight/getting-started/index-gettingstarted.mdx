...
RedisInsight is an intuitive and efficient GUI for Redis, allowing you to interact with your databases and manage your data—with built-in support for Redis Stack. It is a 100% free Redis GUI tool that allows you to visualise, monitor, and optimize while developing your applications with Redis.

![my image](redisinsight.gif)

RedisInsight provides built-in support for the JSON, Search, Graph, Redis Streams, and Time Series data structures to make it even easier to query, visualize, and interactively manipulate search indexes, graphs, streams, and time-series data.
...
To use RedisInsight on Mac OS, download it from the RedisInsight page on the Redis website:
...
If you're using Redis Stack, you don't really need to install RedisInsight separately.
Redis Stack includes RedisInsight, a visualization tool for understanding and optimizing Redis data.
...
#### Step 2. Run Redis Stack Docker image
...
- [Debugging Redis using Slowlog Configuration under RedisInsight](/explore/redisinsight/slowlog)
...
- [Manage Your Redis Cluster using RedisInsight](/explore/redisinsight/cluster)
