---
id: index-cluster
title: Manage Your Redis Cluster using RedisInsight GUI
sidebar_label: Manage Your Redis Cluster 
...
There are two kinds of nodes: master and slave nodes. To get started with Redis Cluster, follow the below steps to build 3 Master and 3 Replicas.
...
In order to start a 6 nodes cluster with 3 masters and 3 slaves, just type the following commands:
...
## Step 6: Connecting to Redis database

[Follow this link](https://developer.redislabs.com/explore/redisinsight/) to install RedisInsight on your local system.
...
- Master-Replica Layout - This view contains masters along with their replicas. This view contains information about slots ranges, host, port, etc for both master and replica.
...
Let us add a new node by using the below redis.conf file:
...
To delete a node, select “Master-Replica Nodes” option and you will see all the slave nodes
...
In order to upgrade the Redis process of one of the master nodes it is a good idea to failover it in order to turn it into a slave with minimal impact on availability.
