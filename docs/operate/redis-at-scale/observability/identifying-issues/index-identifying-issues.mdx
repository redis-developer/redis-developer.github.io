...
<hr/>
<strong>Big Keys:</strong> This option will scan the dataset for big keys and provide information about them.
...
<hr/>
<strong>Mem Keys:</strong> Similarly to big keys mem keys will look for the biggest keys but also report on the average sizes.
...
```
<hr/>
<strong>Hot Keys:</strong> The hot keys scan is only available when the `maxmemory-policy` is set to `volatile-lfu` or `allkeys-lfu`. If you need to identity hot keys you can add this argument to redis-cli.
...
```
<hr/>
<strong>Monitor:</strong> The `MONITOR` command allows you to see a stream of every command running against your Redis instance.
